...
\section{Definizione della semantica di BExp}
[Da scrivere.]

\section{Definizione della semantica di AExp} \marginpar{Marano}
...
$ \denot{\calE}{n}{s} = \llbracket n \rrbracket_{Num}$
...
$ \denot{\calE}{E_1 + E_2}{s} = \begin{cases}
	...
	\end{cases}
$
Vedremo in seguito come poter eliminare questa e altre funzioni per casi definendo semplicemente la somma e altri operatori in $\Nset_\bot$.
...
%$$
%\denot{\calC}{x \weq E}{s} = \begin{cases}
%	\assign{s}{x}{\denot{\calE}{E}{s}} & \text{se } \denot{\calE}{E}{s} \neq \bot;\\
%	\bot & \text{altrimenti};
%	\end{cases}
%
%\denot{\calC}{\kw{skip}}{s} = s
%
%\denot{\calC}{C_1 ; C_2}{s} = \begin{cases}
%	\bot & \text{se } \denot{\calC}{C_1}{s} = \bot;\\
%	\denot{\calC}{C_2}{\denot{\calC}{C_1}{s}} & \text{altrimenti};\\
%	\end{cases}
%
%\denot{\calC}{\condif{B}{C_1}{C_2}}{s} = \begin{cases}
%	\denot{\calC}{C_1}{s} & \text{se } \denot{\calB}{B}{s} = \tt ;\\
%	\denot{\calC}{C_2}{s} & \text{se } \denot{\calB}{B}{s} = \ff ;\\
%	\bot & \text{altrimenti};
%	\end{cases}
%
...
$$\fund{\conadd}{\Nset_\bot \times \Nset_\bot}{\Nset_\bot} = \lambda n_1, n_2 \in \Nset_\bot . \begin{cases}
...
%\mathrm{seq}(f,g)(s) = \begin{cases}
%	\bot & \text{se } f(s) = \bot;\\
%	g(f(s)) & \text{altrimenti};
%	\end{cases}\\
%	
%\fund{\mathrm{cond}}{P \times \ST \times \ST}{\ST}
%\mathrm{cond}(p,f,g)(s) = \begin{cases}
%	f(s) & \text{se } p(s) = \tt;\\
%	g(s) & \text{se } p(s) = \ff;\\
%	\bot & \text{altrimenti};
%	\end{cases}\\
...
Otteniamo quindi, definendo $\fund{F}{\ST}{\ST}$ come $F(f) = \mathrm{cond}(\calB \llbracket B \rrbracket, \mathrm{seq}(\calC \llbracket C \rrbracket, f), \mathrm{id})$ che $f$ è un punto fisso di $F$, ovvero che per trovare la semantica del comando while ci basta trovare il punto fisso di tale funzione, per fare ciò sfrutteremo il Teorema di Tarski visto precedentemente.
