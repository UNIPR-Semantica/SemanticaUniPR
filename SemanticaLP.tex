...
\begin{esercizio}
  Sia $(C, \preceq)$ un PO $: Chain(C) \land |C| < \aleph_0 \Rightarrow \exists max\ C$
  \begin{proof}
...
\begin{esercizio}
  Sia $(C, \preceq)$ un PO $: Chain(C) \Rightarrow (X \subseteq C \Rightarrow Chain(X))$
  \begin{proof}
...
\begin{proof}
  ...
  \end{enumerate}
  Dato che \emph{X} è una catena, $f_1 \sqsubseteq f_2 \lor f_2 \sqsubseteq f_1 $. E otteniamo nel primo caso la (1) mentre la (2) nel secondo caso. Siaccome siamo in un CPO concludiamo che il lub esiste. \\
